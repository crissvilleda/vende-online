...
const ComprarForm = (props) => {
    const { CancelarCompra, producto, handleSubmit } = props;
    return (
        <form onSubmit={handleSubmit} className="container__comprar">
            <div>
                <h3 style={{fontSize: "18px", marginBottom: "5px"}}>Detalle Compra</h3>
                ...
                <Field name="cantidad" type="number" component={renderField} />
            </div>
            ...
            <div className="uk-flex uk-flex-between uk-margin-top">
                <button className="btn btn-primary btn-sm" type="submit">Comprar</button>
                <button onClick={() => CancelarCompra()} className="btn btn-secondary btn-sm" type="button">Cancelar</button>
            </div>
        </form>
    );
};
